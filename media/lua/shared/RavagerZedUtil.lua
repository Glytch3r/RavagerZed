...
function RavagerZed.findzedID(int)
...
function RavagerZed.normalizeTo100(x, min, max)
    x = math.max(min, math.min(x, max))
    return ((x - min) / (max - min)) * 100
end

-----------------------            ---------------------------
function RavagerZed.checkDist(pl, sq)
	local dist = pl:DistTo(sq:getX(), sq:getY())
    return math.floor(dist)
end

function RavagerZed.isWithinRange(pl, zed, range)
	local dist = pl:DistTo(zed:getX(), zed:getY())
    return dist <= range
end

function RavagerZed.isClosestPl(pl, zed)
	if not RavagerZed.isRavagerZed(zed) then return end
	local plDist = RavagerZed.checkDist(pl, zed)
