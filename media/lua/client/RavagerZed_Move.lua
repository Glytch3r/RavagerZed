...
function RavagerZed.isCrawler(zed)
...
function RavagerZed.isWalkable(sq)
...
function RavagerZed.stopWalking(targ)
	targ:getPathFindBehavior2():reset();
end



function RavagerZed.isStepped(zed)
...
function RavagerZed.isShouldStand(zed)
...
function RavagerZed.setStand(zed)
...
function RavagerZed.getWalkType(zed)
	return tostring(zed:getVariableString("zombieWalkType"))
end

function RavagerZed.getWalkNum(zed)
	local walk = RavagerZed.getWalkType(zed)
...
function RavagerZed.isMoving(zed)
	if not zed then return false end
	--if not RavagerZed.isRavagerZed(zed) then return false end
	return zed:getVariableBoolean('bMoving')
end

function RavagerZed.turnAround(targ)
	local x = targ:getX()
	local y = targ:getY()
...
function RavagerZed.checkWalkableSq()
	local pl = getPlayer()
	...
	if not pl:isOutside() then
		for i=0, rmSqs:size()-1 do
			...
			for _, testSq in ipairs(tab) do
				if testSq  then
					local isGood = false
					if RavagerZed.isWalkable(testSq) then
						isGood = true
					end
					RavagerZed.TempMarker(testSq, isGood)
				end
			end
		end
	end
